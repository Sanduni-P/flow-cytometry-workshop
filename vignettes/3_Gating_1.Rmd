...
The Gating sections of this workshop is designed as follows:
...
```{r load_library, message=FALSE, echo = TRUE, eval=TRUE, message=FALSE, warning=FALSE}
library(flowWorkspace)
library(ggcyto)
library(openCyto)
```

To create a `GatingSet`, first load in a `cytoset`. 
...
metadata <- data.frame(name = sampleNames(cs),
                       ...
                       panel = ifelse(
                         grepl(pattern = "TNK",x = sampleNames(cs)),
                         "T Cell Panel","Myeloid Panel"), 
...
*Note*: The approach to **compensate** and **transform** a `GatingSet` is similar to that of `cytoframe` and `cytoset`.
...
2. How would you be able to tell if a `GatingSet` has been transformed?
...
3. What is the difference between `gs[1]` and `gs[[1]]`?
